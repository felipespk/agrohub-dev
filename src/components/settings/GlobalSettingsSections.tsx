...
import { Building2, Save, User, ShieldCheck, Lock, Eye, EyeOff } from "lucide-react";
        ...
        <CardTitle className="flex items-center gap-2 text-[20px]">
          ...
          {hasPassword ? (
            <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-700">
              <Lock className="h-3 w-3" /> Ativa
            </span>
          ) : (
            <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-500">
              Inativa
            </span>
          )}
        </CardTitle>
        <CardDescription>Configure a Senha Master para proteger edições/exclusões de registros com mais de 48 horas.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="max-w-md space-y-4">
          <div className="rounded-lg bg-[#F3F4F6] p-4 text-sm space-y-1">
