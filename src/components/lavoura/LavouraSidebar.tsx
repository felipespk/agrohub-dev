...
const menuItems = [
  { title: "Dashboard", url: "/lavoura", icon: LayoutDashboard },
  { title: "Talhões", url: "/lavoura/talhoes", icon: Map },
  { title: "Safras", url: "/lavoura/safras", icon: Calendar },
  { title: "Caderno de Campo", url: "/lavoura/atividades", icon: BookOpen },
...
const configItems = [
  { title: "Configurações", url: "/lavoura/configuracoes", icon: Settings },
];
    ...
    items.map((item) => (
      <SidebarMenuItem key={item.title}>
        <SidebarMenuButton asChild>
          <NavLink to={item.url} end={item.url === "/lavoura"}
      ...
      <SidebarContent className="flex flex-col h-full bg-[hsl(var(--sidebar-background))]">
        <div className={`px-6 pt-6 pb-3 ${collapsed ? "items-center flex flex-col px-2" : ""}`}>
          <div className={`flex items-center gap-2.5 ${collapsed ? "justify-center" : ""}`}>
            <img src="/icon-agrohub.png" alt="AgroHub" width={28} height={28} className="shrink-0 rounded" /> src="/icon-agrohub.png" alt="AgroHub" width={28} height={28} className="shrink-0 rounded" /> />
            {!collapsed && <span className="text-[20px] font-bold text-white tracking-tight">AgroHub</span>}
          </div>
          {!collapsed && (
            <button onClick={() => navigate("/lavoura/configuracoes")}
              className="text-[12px] text-[#8A99AF] hover:text-white/80 transition-colors pl-[34px] truncate mt-1 text-left block">
              {getFarmName("lavoura") || "Configurar Fazenda"}
            </button>
          )}
        </div>
