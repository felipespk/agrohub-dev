...
import { saidasMock } from "@/data/mock-data";
import { Truck } from "lucide-react";

export default function ExpedicaoPage() {
  const saidas = saidasMock;
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="kpi-card">
          ...
          <p className="text-2xl font-display font-bold text-foreground">{totalSacos.toLocaleString("pt-BR")}</p>
        </div>
        <div className="kpi-card">
          <p className="text-xs text-muted-foreground">Total de Kgs</p>
          <p className="text-2xl font-display font-bold text-foreground">{totalKgs.toLocaleString("pt-BR")}</p>
        </div>
        <div className="kpi-card">
          <p className="text-xs text-muted-foreground">Total de Toneladas</p>
          <p className="text-2xl font-display font-bold text-primary">{fmt(totalToneladas)}</p>
        </div>
      </div>
